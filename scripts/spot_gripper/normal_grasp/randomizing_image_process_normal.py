...
views = {
    "default": {
        "depth": "depth.npy",
        "normal_map": "normal_map.npy",
        "coords": "cylinder_pixel_coords.npy",
        "cam_pos": [3, -1.5, 0.2],
        "up": [0, 0, 1]
    },
    "top": {
...
view_name = random.choice(list(views.keys()))
view = views[view_name]
print(f"Selected view: \033[1;92m{view_name}\033[0m")

# Load data for the selected view
depth = np.load(output_dir / view["depth"])
normal_map = np.load(output_dir / view["normal_map"])
cylinder_pixel_coords = np.load(output_dir / view["coords"])
...
print(f"Random pixel coordinate from {view_name} view: ({random_x}, {random_y})")
...
input("\nEnter to proceed with alignment...\n")
for _ in range(200):
    scene.step()
