...
# Cylinder's position cannot be changed so that it may be faithful to the saved data.
...
cylinder_pixel_coords = np.load(output_dir / "cylinder_pixel_coords.npy")
...
# Convert normal from [0, 255] to [-1, 1] - Assuming normal_map is uint8 [0, 255] per channel
vector_normal = 2 * (specific_point_normal - 127.5) / 255
print(f"Normal vector at center pixel: {vector_normal}")
...
value = pixel_to_world(random_x, random_y, specific_point_depth, cam_pos, cam_lookat, fov, img_width, img_height)

# Optional: Verify data shapes and ranges
print(f"Depth shape: {depth.shape}")
print(f"Normal map shape: {normal_map.shape}")
print(f"Depth at center: {specific_point_depth}")
...
print(f"World coordinates at center pixel Point: \033[1;92m{value}\033[0m")
input("\nEnter. . . \n")
for _ in range(200):
    scene.step()
# Align the spot with Normal Direction obtained
