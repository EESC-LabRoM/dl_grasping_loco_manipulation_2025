...
spot_gripper = scene.add_entity(
    gs.morphs.URDF(
        ...
        euler=(0, 90, 0),  # Aligned for Z-axis grasp
        pos=(0.0, 0.0, 0.30),  # Initial position
        scale=1.0,
...
reset_cylinder()
spot_gripper.set_pos(np.array([0.0, 0.0, 0.30]), zero_velocity=True)
# Test grasp at varying X positions
print("\033[1;92mTesting X translate position grasping\033[0m")
x_positions = np.arange(-cylinder_radius, cylinder_radius + 0.01, 0.01)
for x in x_positions:
    print(f"\nTesting grasp at position \033[1;94m[{x:.2f}, 0, 0.3]\033[0m")
    # Move gripper to new X position
    spot_gripper.set_pos(np.array([x, 0.0, 0.30]), zero_velocity=True)
