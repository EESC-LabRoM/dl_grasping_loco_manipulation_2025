...
z1.set_dofs_kp(
    kp             = np.array([4500, 4500, 3500, 3500, 2000, 2000, 100]),
    dofs_idx_local = dofs_idx,
)
# set velocity gains
z1.set_dofs_kv(
    kv             = np.array([450, 450, 350, 350, 200, 200, 10]),
    dofs_idx_local = dofs_idx,
)
# set force range for safety
z1.set_dofs_force_range(
    lower          = np.array([-87, -87, -87, -87, -12, -12, -100]),
    upper          = np.array([ 87,  87,  87,  87,  12,  12, 100]),
    dofs_idx_local = dofs_idx,
)
# Hard reset
for i in range(150):
    if i < 50:
        z1.set_dofs_position(np.array([1, 1, 0, 0, 0, 0, 0.04]), dofs_idx)
    elif i < 100:
        z1.set_dofs_position(np.array([-1, 0.8, 1, -2, 1, 0.5, 0.04]), dofs_idx)
    else:
        z1.set_dofs_position(np.array([0, 0, 0, 0, 0, 0, 0]), dofs_idx)
...
for i in range(1250):
    if i == 0:
        z1.control_dofs_position(
            np.array([1, 1, 0, 0, 0, 0, 0.04]),
            dofs_idx,
        )
    elif i == 250:
        z1.control_dofs_position(
            np.array([-1, 0.8, 1, -2, 1, 0.5, 0.04]),
            dofs_idx,
        )
    elif i == 500:
        z1.control_dofs_position(
    ...
    cam.render()
cam.stop_recording(save_to_filename='z1_no_stator_video.mp4', fps=60)