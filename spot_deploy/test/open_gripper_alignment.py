...
def vector_to_euler(vec):
    ...
    print(f"Local Z-axis:\n \033[1;92m({rotation.apply(z_axis)})\033[0m \n")
    print(rotation)
...
def align_gripper(robot, command_client, position, normal_vector, offset, shot):
    try:
        # Get the Rigid Body Transformation from hand to body
        ...
        print(f"Original normal: {normal_vector}")  # points to spot
        print(
            f"Flipped normal: \033[1;92m{flipped_normal}\033[0m"
        ...
        # Check if roll is close to ±90° (π/2 or -π/2)
        # rotation_quat = math_helpers.Quat(w=rotation.w, x=rotation.x, y=rotation.y, z=rotation.z)
        # roll_angle = rotation_quat.to_pitch()
        # tolerance = 0.2  # ~11.5° tolerance
        # target_roll = np.pi / 2 
        # # If not close to ±90°
        # if abs(abs(roll_angle) - target_roll) > tolerance:  
        #     print("Roll not near ±90°, applying 90° X rotation.")
        #     roll_quat = math_helpers.Quat.from_pitch(angle=np.pi/2)
        #     # Add 90° roll
        #     full_rotation = rotation_quat * roll_quat  
        #     full_rotation = geometry_pb2.Quaternion(w=full_rotation.w, x=full_rotation.x, y=full_rotation.y, z=full_rotation.z)
        # else:
        #     print("Roll already near ±90°, no extra rotation needed.")
        #     full_rotation = rotation 

        # print(
        #     f"Final rotation (with 90° X): \033[1;92m(w={full_rotation.w:.3f}, x={full_rotation.x:.3f}, y={full_rotation.y:.3f}, z={full_rotation.z:.3f})\033[0m"
        # )
        # print(
        #     f"In Euler(rad): \033[1;92m{math_helpers.quat_to_eulerZYX(full_rotation)}\033[0m"
        # )
        ...
        # Applying the Transfomation - converting the pose from hand to body
        vector = (
            camera_tform_body
        ...
        print(
            f"\nTarget position (vector position): \033[1;92m(x={vector.x:.3f}, y={vector.y:.3f}, z={vector.z:.3f})\033[0m"
        )
        print(
            f"Target rotation (vector orientation): \033[1;92m(w={vector.rot.w:.3f}, x={vector.rot.x:.3f}, y={vector.rot.y:.3f}, z={vector.rot.z:.3f})\033[0m."
        )
        print(
...
def main():
    ...
    with LeaseKeepAlive(lease_client, must_acquire=True, return_at_exit=True):
        ...
        success = align_gripper(
            robot, command_client, position, normal_vector, 0, rgb_response.shot
        )
        ...
        align_gripper(robot, command_client, position, normal_vector, 0.05, rgb_response.shot)
        time.sleep(2)
