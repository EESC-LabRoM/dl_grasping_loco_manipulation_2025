...
from bosdyn.api import image_pb2, geometry_pb2, arm_command_pb2, trajectory_pb2
...
def align_gripper(robot, command_client, position, normal_vector, shot):
    ...
    try:
        # Get transform from hand_color_image_sensor to body
        ...
        print(
            f"\nTarget rotation obtained by d2nt withou rotate: w={rotation.w:.3f}, x={rotation.x:.3f}, y={rotation.y:.3f}, z={rotation.z:.3f}."
        )
        print(f"In Euler: {math_helpers.quat_to_eulerZYX(rotation)}")

        print(
            f"\nTarget rotation obtained by d2nt: w={rotation.w:.3f}, x={rotation.x:.3f}, y={rotation.y:.3f}, z={rotation.z:.3f}."
        )
        print(f"In Euler: {math_helpers.quat_to_eulerZYX(rotation)}")
        ...
        if distance > 2:
            raise ValueError(f"Target pose ({distance:.2f} m) exceeds arm reach (~2 m)")

        print(
            f"\nTarget position: x={vector.x:.3f}, y={vector.y:.3f}, z={vector.z:.3f}"
        )

        print(
            f"\nTarget vector rotation: w={vector.rot.w:.3f}, x={vector.rot.x:.3f}, y={vector.rot.y:.3f}, z={vector.rot.z:.3f}."
        )
        print(f"In Euler: {math_helpers.quat_to_eulerZYX(vector.rotation)}")
