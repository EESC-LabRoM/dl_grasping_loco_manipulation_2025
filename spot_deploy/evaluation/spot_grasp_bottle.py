...
def preprocess_data(rgb, depth, normal, mask, target):
    ...
    normal = np.copy(normal)  # Copy normal map for safety
    
    print(f"    1. Mask step")
    mask = cv2.inRange(mask, 1, 1).astype(bool).astype(int) # Select only the object, removing the gripper or other elements
    print(f"    2. Depth step")    
    depth[mask == 0] = 2 # Place everything that is not the object 2 meters away
...
def align_gripper(robot, command_client, position, normal_vector, offset, distance, shot):
    ...
    try:
        ...
        n_euler = vector_to_euler(normal)
        ...
        command = RobotCommandBuilder.arm_pose_command_from_pose( # TODO: Crack the approach in 2 motions: 1. approach with offset+rotation 2. final approach to grasp  
...
def main():
    ...
    with LeaseKeepAlive(lease_client, must_acquire=True, return_at_exit=True):
        ...
        rgb_input, depth_input, normal_map_input, mask, _ = preprocess_data(
            rgb_img, depth_img, normal_map, seg, target=np.zeros(rgb_img.shape)
        )
        ...
        pixel, normal_vector, position, distance = calculate_params(
            rgb_input, depth_img, grasp_y, grasp_x, normal_map, fx, fy, u0, v0
        )
        if position is None:
        ...
        success = align_gripper(robot, command_client, position, normal_vector, 0.2, distance, shot)
        if success:
            time.sleep(2)
